...
class SaleOrderRecurring(models.Model):
        ...
        for order in orders:
            ...
            try:
                ...
                if payment.status == 'paid':
                    ...
                    order.recurring_next_date = fields.Date.add(fields.Date.today(), months=1)
                    ...
                    _logger.warning("Payment failed for %s, attempt %s", order.name, order.mollie_retry_count)
    ...
    def _send_failure_email(self, order):
        template = self.env.ref('mollie_recurring_payments.mail_template_mollie_failed_payment')
        template.send_mail(order.id, force_send=True)

    def _send_success_email(self, order):
        template = self.env.ref('mollie_recurring_payments.mail_template_mollie_success_payment')
        template.send_mail(order.id, force_send=True)
